...
public sealed class NbtReader : IDisposable
{
    ...
    private void GoDown()
    {
        _nodes ??= new Stack<NbtReaderNode>();
    ...
    public bool ReadToDescendant([CanBeNull] string tagName)
    {
        if (_state == NbtParseState.Error)
            throw new InvalidReaderStateException(ErroneousStateError);
        if (_state == NbtParseState.AtStreamEnd) return false;
    ...
    public int Skip()
    {
        if (_state == NbtParseState.Error)
            throw new InvalidReaderStateException(ErroneousStateError);
        if (_state == NbtParseState.AtStreamEnd) return 0;
    ...
    private static void AddToParent([NotNull] NbtTag thisTag, [NotNull] NbtTag parent)
    {
